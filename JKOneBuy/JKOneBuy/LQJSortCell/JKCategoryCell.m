...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])
    {
        ...
        fontMinisize =[UIScreen mainScreen].bounds.size.width*(12.f/400.f);
...
-(void)buildUI
{
    ...
    _add_shoppingCartBut = [[UIButton alloc] init];
    _add_shoppingCartBut.tag = 172700;
    
    _one_buyBut = [[UIButton alloc] init];
    _one_buyBut.tag =172701;
    ...
    [self addSubview:_add_shoppingCartBut];
    [self addSubview:_one_buyBut];
}
...
-(void)setName_str:(NSString *)name_str
{
    ...
    name_label.font = [UIFont systemFontOfSize:fontsize];
...
-(void)setSum_str:(NSString *)sum_str
{
    ...
    sum_label.font = [UIFont systemFontOfSize: fontsize];
    
    [sum_label setTextColor:[UIColor colorWithRed:0.694 green:0.694 blue:0.698 alpha:1.00]];
...
-(void)setLeft_str:(NSString *)left_str
{
    ...
    left_label.font = [UIFont systemFontOfSize:fontsize];
    [left_label setTextColor:[UIColor colorWithRed:0.694 green:0.694 blue:0.698 alpha:1.00]];
    ...
    [attri addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:0.494 green:0.600 blue:0.984 alpha:1.00] range:rangeFinal];
    
    left_label.attributedText = attri;
    
    
    _add_shoppingCartBut.frame = CGRectMake(name_label.frame.origin.x + name_label.frame.size.width,14 * kFloatWidthScale,72 * kFloatWidthScale,22 * kFloatWidthScale);
    _add_shoppingCartBut.backgroundColor = [UIColor whiteColor];
    [_add_shoppingCartBut setTitleColor:[UIColor colorWithRed:0.906 green:0.278 blue:0.494 alpha:1.00] forState:UIControlStateNormal];
    
    _add_shoppingCartBut.alpha =1;
    _add_shoppingCartBut.tintColor = [UIColor blackColor];
    [_add_shoppingCartBut setTitle:@"加入购物车" forState:UIControlStateNormal];
    [_add_shoppingCartBut setFont:[UIFont systemFontOfSize:fontsize]];
    
    [_add_shoppingCartBut setTitle:@"已经加入购物车" forState:UIControlStateSelected];
    _add_shoppingCartBut.layer.borderWidth = 5;
    _add_shoppingCartBut.layer.borderColor = (__bridge CGColorRef _Nullable)([UIColor colorWithRed:0.906 green:0.278 blue:0.494 alpha:1.00]);
    _add_shoppingCartBut.layer.masksToBounds=YES;
    
    
    _add_shoppingCartBut.layer.cornerRadius = 5;
    [_add_shoppingCartBut addTarget: self action:@selector(buttonClicked:) forControlEvents:UIControlEventTouchUpInside];
    
    
    _one_buyBut.frame = CGRectMake(name_label.frame.origin.x + name_label.frame.size.width,40 * kFloatWidthScale,72 * kFloatWidthScale,22 * kFloatWidthScale);
    [_one_buyBut setTitle:@"一元抢购" forState:UIControlStateNormal];
    _one_buyBut.backgroundColor = [UIColor colorWithRed:0.906 green:0.278 blue:0.494 alpha:1.00];
    
    [_one_buyBut setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    _one_buyBut.layer.cornerRadius = 5;
    [_one_buyBut setFont:[UIFont systemFontOfSize:fontsize]];
    
    [_one_buyBut addTarget: self action:@selector(buttonClicked:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)resetButtonStates:(BOOL)states
{
    ...
    if (states) {
        _add_shoppingCartBut.selected = YES;
        _add_shoppingCartBut.userInteractionEnabled = NO;
        _add_shoppingCartBut.font = [UIFont systemFontOfSize:fontMinisize];
    }
    else
    {
...
{
    UITableViewCell * cellTemp = (UITableViewCell*)[imageView superview] ;
