//
//  LQJJKSortViewController.h
//  JKOneBuy
//
//  Created by teaplant on 16/6/23.
//  Copyright © 2016年 teaplant. All rights reserved.
//

#import <UIKit/UIKit.h>

@interface LQJJKSortViewController : UIViewController<UITableViewDelegate,UITableViewDataSource>{
}
@property(nonatomic,strong) UITableView *sortTableView;
@property (nonatomic ,copy) NSString * str_url;
@property ( nonatomic ,copy)NSString * name_sort;

@end
